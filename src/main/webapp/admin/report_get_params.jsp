        ...
        <table class="results-old" width="50%">
          ...
          <c:choose>
            <c:when test="${empty params}">
              <tr><td colspan="4" align="center" style="color: red">No parameters defined</td></tr>
            </c:when>
            <c:otherwise>
          ...
          <tr class="fuzzy">
            <td colspan="5" align="right">
              <button onclick="$('#format').val('<c:out value="${ReportUtils.OUTPUT_PDF}"/>')" class="button_execute_report_renew"><img src="img/action/pdf.png" alt="Generate PDF" title="Generate PDF"/></button>
              &nbsp;
              <button onclick="$('#format').val('<c:out value="${ReportUtils.OUTPUT_RTF}"/>')" class="button_execute_report_renew"><img src="img/action/rtf.png" alt="Generate RTF" title="Generate RTF"/></button>
              &nbsp;
              <button onclick="$('#format').val('<c:out value="${ReportUtils.OUTPUT_CSV}"/>')" class="button_execute_report_renew"><img src="img/action/table.png" alt="Generate CSV" title="Generate CSV"/></button>
            </td>
          </tr>
        </table>
      </form>

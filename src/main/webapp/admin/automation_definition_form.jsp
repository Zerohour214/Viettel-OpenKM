          ...
          <td colspan="2" align="right">
            <c:choose>
              <c:when test="${action=='createValidation' || action=='deleteValidation' || action=='editValidation'}">
                <input type="button" onclick="javascript:hideValidation()" value="Cancel" class="noButton btn btn-warning"/>
              </c:when>
              <c:when test="${action=='createAction' || action=='deleteAction' || action=='editAction'}">
                <input type="button" onclick="javascript:hideAction()" value="Cancel" class="noButton btn btn-warning"/>
              </c:when>
            </c:choose>
            <c:choose>
              <c:when test="${action=='createValidation' || action=='createAction'}">
              	<input type="submit" value="Create" class="yesButton btn btn-success"/>
              </c:when>
              <c:when test="${action == 'editAction' || action == 'editValidation'}">
              	<input type="submit" value="Edit" class="yesButton btn btn-success"/>
              </c:when>
              <c:when test="${action == 'deleteAction' || action == 'deleteValidation'}">
              	<input type="submit" value="Delete" class="yesButton btn btn-success"/>
              </c:when>
            </c:choose>
          </td>
        </tr>

          ...
          <li id="${cat.uuid}" data-action="fldMenu">
            ...
            <c:choose>
              <c:when test="${u:bitwiseAnd(cat.permissions, GWTPermission.WRITE) == GWTPermission.WRITE}">
                <c:choose>
              	  <c:when test="${cat.hasChildren}"><c:set var="fldImg" value="glyphicons glyphicons-folder-new icon-green-renew"/></c:when>
              	  <c:otherwise><c:set var="fldImg" value="glyphicons glyphicons-folder-open icon-green-renew"/></c:otherwise>
            	</c:choose>
              </c:when>
              <c:otherwise>
                <c:choose>
              	  <c:when test="${cat.hasChildren}"><c:set var="fldImg" value="glyphicons glyphicons-folder-new icon-red-renew"/></c:when>
              	  <c:otherwise><c:set var="fldImg" value="glyphicons glyphicons-folder-open icon-red-renew"/></c:otherwise>
            	</c:choose>
              </c:otherwise>
            </c:choose>
            <a href="${urlBrowse}"><span class="${fldImg} ui-li-icon"></span><u:getName path="${cat.path}"/></a>
            <a href="${urlMenu}"></a>
          </li>
        </c:forEach>
          ...
          <li id="${fld.uuid}" data-action="fldMenu">
            ...
            <c:choose>
              <c:when test="${u:bitwiseAnd(fld.permissions, GWTPermission.WRITE) == GWTPermission.WRITE}">
                <c:choose>
              	  <c:when test="${fld.hasChildren}"><c:set var="fldImg" value="glyphicons glyphicons-folder-new icon-green-renew"/></c:when>
              	  <c:otherwise><c:set var="fldImg" value="glyphicons glyphicons-folder-open icon-green-renew"/></c:otherwise>
            	</c:choose>
              </c:when>
              <c:otherwise>
                <c:choose>
              	  <c:when test="${fld.hasChildren}"><c:set var="fldImg" value="glyphicons glyphicons-folder-new icon-red-renew"/></c:when>
              	  <c:otherwise><c:set var="fldImg" value="glyphicons glyphicons-folder-open icon-red-renew"/></c:otherwise>
            	</c:choose>
              </c:otherwise>
            </c:choose>
            <c:choose>
              <c:when test="${context == 'context.title.categories'}">
                <a href="#"><span class="${fldImg} ui-li-icon"></span><u:getName path="${fld.path}"/></a>
              </c:when>
              <c:otherwise>

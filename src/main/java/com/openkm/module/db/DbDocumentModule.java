...
public class DbDocumentModule implements DocumentModule {
	...
	public Document create(String token, Document doc, InputStream is, long size, String userId, Ref<FileUploadResponse> fuResponse)
		...
		try {
			...
			if (!name.isEmpty()) {
				...
				vtProperties.put("docCode", doc.getDocCode());
				vtProperties.put("docName", doc.getDocName());
				vtProperties.put("docEffectiveDate", doc.getDocEffectiveDate());
				vtProperties.put("docOrgId", doc.getDocOrgId());
				vtProperties.put("docConfidentiality", doc.getDocConfidentiality());
				vtProperties.put("docExpiredDate", doc.getDocExpiredDate());
				vtProperties.put("docAuthor", doc.getDocAuthor());
				vtProperties.put("publishCom", doc.getPublishCom());
				vtProperties.put("publishedYear", doc.getPublisedYear());
				vtProperties.put("pageNumber", doc.getPageNumber());

				// Create node
				Set<String> keywords = doc.getKeywords() != null ? doc.getKeywords() : new HashSet<String>();
				NodeDocument docNode = BaseDocumentModule.create(auth.getName(), parentPath, parentNode, name, doc.getTitle(), vtProperties,
	...
	@Override
	public Version checkin(String token, String docId, InputStream is, String comment, HashMap<String, Object> vtProperties) throws FileSizeExceededException,
		...
		return checkin(token, docId, is, is.available(), comment, null, 0, vtProperties);
	}

	@Override
	public Version checkin(String token, String docId, InputStream is, String comment, int increment, HashMap<String, Object> vtProperties) throws FileSizeExceededException,
			UserQuotaExceededException, VirusDetectedException, AccessDeniedException, RepositoryException, PathNotFoundException,
			LockException, VersionException, IOException, DatabaseException, AutomationException {
		return checkin(token, docId, is, is.available(), comment, null, increment, vtProperties);
	}

	/**
	 * Used in Zoho extension
	 */
	public Version checkin(String token, String docId, InputStream is, String comment, String userId, HashMap<String, Object> vtProperties) throws FileSizeExceededException,
			UserQuotaExceededException, VirusDetectedException, AccessDeniedException, RepositoryException, PathNotFoundException,
			LockException, VersionException, IOException, DatabaseException, AutomationException {
		return checkin(token, docId, is, is.available(), comment, userId, 0, vtProperties);
	}

	/**
	 * Used when big files and WebDAV
	 */
	public Version checkin(String token, String docId, InputStream is, long size, String comment, String userId, HashMap<String, Object> vtProperties)
			...
			AutomationException {
		return checkin(token, docId, is, size, comment, userId, 0, vtProperties);
	}

	/**
	 * Used when increase document major version
	 */
	public Version checkin(String token, String docId, InputStream is, long size, String comment, String userId, int increment, HashMap<String, Object> vtProperties)
			throws FileSizeExceededException, UserQuotaExceededException, VirusDetectedException, AccessDeniedException,
			RepositoryException, PathNotFoundException, LockException, VersionException, IOException, DatabaseException,
			AutomationException {
		...
		try {
			...
			NodeDocumentVersion newDocVersion = NodeDocumentVersionDAO.getInstance().checkin(userId, comment, docUuid, is, size, increment, vtProperties);

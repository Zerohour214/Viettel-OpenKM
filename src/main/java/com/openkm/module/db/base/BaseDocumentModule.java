...
public class BaseDocumentModule {
	...
	public static NodeDocument create(String user, String parentPath, NodeBase parentNode, String name, String title, HashMap<String, Object> vtProperties, Calendar created,
		...
		if (vtProperties.get("docCode") != null){
			documentNode.setDocCode(vtProperties.get("docCode").toString());
		}
		if (vtProperties.get("docName") != null){
			documentNode.setDocName(vtProperties.get("docName").toString());
		}
		if (vtProperties.get("docEffectiveDate") != null){
			documentNode.setDocEffectiveDate((Calendar) vtProperties.get("docEffectiveDate"));
		}
		if (vtProperties.get("docOrgId") != null) {
			documentNode.setDocOrgId(Long.parseLong(vtProperties.get("docOrgId").toString()));
		}
		if (vtProperties.get("docConfidentiality") != null) {
			documentNode.setDocConfidentiality(Integer.parseInt(vtProperties.get("docConfidentiality").toString()));
		}
		if (vtProperties.get("docExpiredDate") != null){
			documentNode.setDocExpiredDate((Calendar) vtProperties.get("docExpiredDate"));
		}
		if (vtProperties.get("docAuthor") != null){
			documentNode.setDocAuthor(vtProperties.get("docAuthor").toString());
		}
		if (vtProperties.get("publishCom") != null){
			documentNode.setPublishCom(vtProperties.get("publishCom").toString());
		}
		if (vtProperties.get("publishedYear") != null){
			documentNode.setPublishedYear((Integer) vtProperties.get("publishedYear"));
		}
		if (vtProperties.get("pageNumber") != null){
			documentNode.setPageNumber((Integer) vtProperties.get("pageNumber"));
		}
	...
	public static Document getProperties(String user, NodeDocument nDocument) throws PathNotFoundException, DatabaseException {
		...
		//Viettel properties
		doc.setDocCode(nDocument.getDocCode());
		doc.setDocName(nDocument.getDocName());
		doc.setDocEffectiveDate(nDocument.getDocEffectiveDate());
		if (nDocument.getDocOrgId() != null) {
			doc.setDocOrgId(nDocument.getDocOrgId());
		}
		else {
			doc.setDocOrgId(0);
		}
		if (nDocument.getDocConfidentiality() != null) {
			doc.setDocConfidentiality(nDocument.getDocConfidentiality());
		}
		else {
		...
		if(nDocument.getPageNumber() != null){
			doc.setPageNumber(nDocument.getPageNumber());
		}
	...
	public static NodeDocument copy(String user, NodeDocument srcDocNode, String dstPath, NodeBase dstNode, String docName,
		...
		try {
			...
			vtProperties.put("docCode", srcDocNode.getDocCode());
			vtProperties.put("docName", srcDocNode.getDocName());
			vtProperties.put("docEffectiveDate", srcDocNode.getDocEffectiveDate());
			vtProperties.put("docExpiredDate", srcDocNode.getDocExpiredDate());
			vtProperties.put("docAuthor", srcDocNode.getDocAuthor());
			vtProperties.put("publishCom", srcDocNode.getPublishCom());
			vtProperties.put("publishedYear", srcDocNode.getPublishedYear());
			vtProperties.put("pageNumber", srcDocNode.getPageNumber());
			...
			newDocument = create(user, dstPath, dstNode, docName, srcDocNode.getTitle(), vtProperties, Calendar.getInstance(), srcDocNode.getMimeType(),

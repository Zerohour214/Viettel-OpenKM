...
public class DbSearchModule implements SearchModule {
	...
	public Query prepareStatement(QueryParams params) throws IOException, ParseException, RepositoryException, DatabaseException {
		...
		params.setDocCode(params.getDocCode() != null ? params.getDocCode().trim() : "");
		params.setDocName(params.getDocName() != null ? params.getDocName().trim() : "");
		params.setDocAuthor(params.getDocAuthor() != null ? params.getDocAuthor().trim() : "");
		params.setPublishCom(params.getPublishCom() != null ? params.getPublishCom().trim() : "");
		...
		if (document) {
			...
			// vt properties
			if (!params.getDocCode().isEmpty()) {
				if (!params.getDocCode().contains("*") && !params.getDocCode().contains("?")) {
					params.setDocCode("*" + params.getDocCode() + "*");
				}

				Term t = new Term("docCode", PathUtils.encodeEntities(params.getDocCode().toLowerCase()));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}

			if (!params.getDocName().isEmpty()) {
				if (!params.getDocName().contains("*") && !params.getDocName().contains("?")) {
					params.setDocName("*" + params.getDocName() + "*");
				}
				Term t = new Term("docName", PathUtils.encodeEntities(params.getDocName().toLowerCase()));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}

			if (params.getDocEffectiveDate() != null) {
				Date effective = params.getDocEffectiveDate().getTime();
				String sEffective = DAY_FORMAT.format(effective);
				Term t = new Term("docEffectiveDate", sEffective);
				queryDocument.add(new TermQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getDocOrgId() > 0) {
				Term t = new Term("docOrgId", String.valueOf(params.getDocOrgId()));
				queryDocument.add(new TermQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getDocExpiredDate() != null) {
				Date expired = params.getDocExpiredDate().getTime();
				String sExpired = DAY_FORMAT.format(expired);
				Term t = new Term("docExpiredDate", sExpired);
				queryDocument.add(new TermQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getDocAuthor() != null && !params.getDocAuthor().isEmpty()) {

				if ( !params.getDocAuthor().isEmpty() && (!params.getDocAuthor().contains("*") && !params.getDocAuthor().contains("?"))) {
					params.setDocAuthor("*" + params.getDocAuthor() + "*");
				}
				Term t = new Term("docAuthor", PathUtils.encodeEntities(params.getDocAuthor().toLowerCase()));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getPublishCom() != null && !params.getPublishCom().isEmpty()) {
				if (!params.getPublishCom().isEmpty() && !params.getPublishCom().contains("*") && !params.getPublishCom().contains("?")) {
					params.setPublishCom("*" + params.getPublishCom() + "*");
				}
				Term t = new Term("publishCom", PathUtils.encodeEntities(params.getPublishCom().toLowerCase()));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getPublishedYear() != null && params.getPublishedYear() != 0) {
				String str = String.valueOf(params.getPublishedYear()).toLowerCase();
				if (!str.isEmpty() && !str.contains("*") && !str.contains("?")) {
					str = "*" + str + "*";
				}
				Term t = new Term("publishedYear", PathUtils.encodeEntities(str));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}
			if (params.getPageNumber() != null && params.getPageNumber() != 0) {
				String str = String.valueOf(params.getPageNumber()).toLowerCase();
				if (!str.isEmpty() && !str.contains("*") && !str.contains("?")) {
					str = "*" + str + "*";
				}
				Term t = new Term("pageNumber", PathUtils.encodeEntities(str = "*" + str + "*"));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}

			if (params.getDocConfidentiality() > 0) {
				Term t = new Term("docConfidentiality", String.valueOf(params.getDocConfidentiality()));
				queryDocument.add(new WildcardQuery(t), BooleanClause.Occur.MUST);
			}
			// vt properties end

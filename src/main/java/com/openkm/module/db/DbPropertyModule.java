...
public class DbPropertyModule implements PropertyModule {
	...
	@Override
	public List<String> addListKeyword(String token, String nodeId, List<String> keywords) throws VersionException, LockException, PathNotFoundException,
			AccessDeniedException, RepositoryException, DatabaseException {
		log.debug("addListKeyword({}, {}, {})", new Object[]{token, nodeId, keywords});
		Authentication auth = null, oldAuth = null;
		String nodePath = null;
		String nodeUuid = null;
		...
		log.debug("addListKeyword: {}", keywords);
		return keywords;
	}
	...
	@Override
	public void updateKeyword(String token, String nodePath) throws VersionException, LockException, PathNotFoundException, AccessDeniedException, RepositoryException, DatabaseException {
		log.debug("updateKeyword({}, {})", new Object[]{token, nodePath});
		Authentication auth = null, oldAuth = null;
		String nodeUuid = null;
		...
		try {
			...
			if (PathUtils.isPath(nodePath)) {
				nodeUuid = NodeBaseDAO.getInstance().getUuidFromPath(nodePath);
			} else {
				nodePath = NodeBaseDAO.getInstance().getPathFromUuid(nodePath);
				nodeUuid = nodePath;
			}

//			NodeBase nNode = NodeBaseDAO.getInstance().findByPk(nodeUuid);
			NodeBaseDAO.getInstance().updateKeyword(nodeUuid);

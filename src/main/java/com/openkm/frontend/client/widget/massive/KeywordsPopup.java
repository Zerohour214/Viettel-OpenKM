			...
			public void onKeyUp(KeyUpEvent event) {
				if ((char) KeyCodes.KEY_ENTER == event.getNativeKeyCode() && keyWordsListPending.isEmpty()) {
					...
					for (int i = 0; i < keys.length; i++) {
						if(!keys[i].isEmpty() && !keys[i].equals("")){
							keyWordsListPending.add(keys[i].trim());
						}
					}
//					String key = suggestKey.getText().trim();
...
//					updateKeyword();
		...
		thesaurusImage = new HTML("<span class=\"glyphicons glyphicons-book-open icon-green-renew\" style=\"padding: 5px;\"></span>");
		...
		close = new Button(OkmConstants.ICON_NO_BUTTON + Main.i18n("button.close"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
		...
		close.addStyleName("btn-warning");
		...
		HTML keyWord = new HTML("<b>" + Main.i18n("document.keywords") + "</b>");
		keyWord.setStyleName("keyword-text-renew");
		table.setWidget(0, 0, keyWord);
		...
		HTML keyWordCloud = new HTML("<b>" + Main.i18n("document.keywords.cloud") + "</b>");
		keyWordCloud.setStyleName("keyword-text-renew");
		table.setWidget(2, 0, keyWordCloud);
	...
	public void addPendingKeyWordsList() {
		if (!keyWordsListPending.isEmpty()) {
			...
			listKeys = drawAndAddKeywords(listKeys);
			addListKeywords(listKeys);
		}
	}

	public List<String> drawAndAddKeywords(List<String> listKeywords){
		if (!keyWordsListPending.isEmpty()) {
...
//				addKeyword(keyword);
				...
				drawAndAddKeywords(listKeywords);
	...
	public void updateKeyword(){
		if (Main.get().mainPanel.desktop.browser.fileBrowser.isPanelSelected()) {
			if (Main.get().mainPanel.desktop.browser.fileBrowser.isDocumentSelected()) {
				Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.document.updateKeyword();
			}
		}
	}
	/**
	...
	public void addListKeywords(List<String> listKeys){
		if (Main.get().mainPanel.desktop.browser.fileBrowser.isMassive()) {
			status.setFlagAddKeywords();
			massiveService.addListKeywords(Main.get().mainPanel.desktop.browser.fileBrowser.getAllSelectedPaths(), listKeys,
					new AsyncCallback<Object>() {
						@Override
						public void onSuccess(Object result) {
							if (keyWordsListPending.isEmpty()) {
			...
			if (Main.get().mainPanel.desktop.browser.fileBrowser.isPanelSelected()) {
				if (Main.get().mainPanel.desktop.browser.fileBrowser.isDocumentSelected()) {
					Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.document.addListKeywords(listKeys);
				} else if (Main.get().mainPanel.desktop.browser.fileBrowser.isFolderSelected()) {
					Main.get().mainPanel.desktop.browser.tabMultiple.tabFolder.folder.addListKeywords(listKeys);
				} else if (Main.get().mainPanel.desktop.browser.fileBrowser.isMailSelected()) {
					Main.get().mainPanel.desktop.browser.tabMultiple.tabMail.mail.addListKeywords(listKeys);
				}
			} else {
				// Otherside tree panel is selected
				Main.get().mainPanel.desktop.browser.tabMultiple.tabFolder.folder.addListKeywords(listKeys);

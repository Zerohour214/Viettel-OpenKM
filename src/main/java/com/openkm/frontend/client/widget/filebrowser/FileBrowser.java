...
public class FileBrowser extends Composite implements OriginPanel, HasDocumentEvent, HasFolderEvent, HasMailEvent,
	...
	private int colDocCodeIndex = 0;
	private int colDocNameIndex = 0;
	private int colDocEffectiveDate = 0;
	private int colDocOrgIndex = 0;
	private int colDocConfidentialityIndex = 0;
	private int colDocExpiredDate = 0;
	private int colDocAuthor = 0;
	private int colPublishCom = 0;
	private int colPublishedYear = 0;
	...
	public FileBrowser() {
		...
		table.addStyleName("okm-scrollTable-renew");
		table.addStyleName("okm-wide-padding-header-table");
	...
	public void langRefresh() {
		...
		if (profileFileBrowser.isDocCodeVisible()) {
			headerTable.setHTML(row, colDocCodeIndex, Main.i18n("document.code"));
			fBController.addOrderByItem(Main.i18n("document.code"), String.valueOf(GWTPaginated.COL_DOC_CODE));
		}

		if (profileFileBrowser.isDocNameVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.docName"));
			fBController.addOrderByItem(Main.i18n("document.docName"), String.valueOf(GWTPaginated.COL_DOC_NAME));
		}

		if (profileFileBrowser.isDocEffectiveDateVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.effectiveDate"));
			fBController.addOrderByItem(Main.i18n("document.effectiveDate"), String.valueOf(GWTPaginated.COL_DOC_EFFECTIVE_DATE));
		}

		if (profileFileBrowser.isDocOrgVisible()) {
			headerTable.setHTML(row, colDocOrgIndex, Main.i18n("document.org"));
			fBController.addOrderByItem(Main.i18n("document.org"), String.valueOf(GWTPaginated.COL_DOC_ORG));
		}

		if (profileFileBrowser.isDocConfidentialityVisible()) {
			headerTable.setHTML(row, colDocConfidentialityIndex, Main.i18n("document.confidentiality.level"));
			fBController.addOrderByItem(Main.i18n("document.confidentiality.level"), String.valueOf(GWTPaginated.COL_DOC_CONFIDENTIALITY));
		}

		if (profileFileBrowser.isDocExpiredDateVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.docExpiredDate"));
			fBController.addOrderByItem(Main.i18n("document.docExpiredDate"), String.valueOf(GWTPaginated.COL_DOC_EXPIRED_DATE));
		}

		if (profileFileBrowser.isDocAuthorVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.docAuthor"));
			fBController.addOrderByItem(Main.i18n("document.docAuthor"), String.valueOf(GWTPaginated.COL_DOC_AUTHOR));
		}

		if (profileFileBrowser.isPublishComVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.publishCom"));
			fBController.addOrderByItem(Main.i18n("document.publishCom"), String.valueOf(GWTPaginated.COL_PUBLISH_COM));
		}

		if (profileFileBrowser.isPublishedYearVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.publishedYear"));
			fBController.addOrderByItem(Main.i18n("document.publishedYear"), String.valueOf(GWTPaginated.COL_PUBLISHED_YEAR));
		}

		if (profileFileBrowser.isPageNumberVisible()) {
			headerTable.setHTML(row, colDocNameIndex, Main.i18n("document.pageNumber"));
			fBController.addOrderByItem(Main.i18n("document.pageNumber"), String.valueOf(GWTPaginated.COL_PAGE_NUMBER));
		}
	...
	public void setProfileFileBrowser(GWTProfileFileBrowser profileFileBrowser, GWTProfilePagination profilePagination) {
		...
		if (profileFileBrowser.isMassiveVisible()) {
			...
			final HTML massive = new HTML("<span class=\"glyphicons glyphicons-important-day icon-green-renew\"></span>");
		...
		if (profileFileBrowser.isDocCodeVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.code"));
			colDocCodeIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocCodeWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.code"), String.valueOf(GWTPaginated.COL_DOC_CODE));
		}

		if (profileFileBrowser.isDocNameVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.docName"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocNameWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.docName"), String.valueOf(GWTPaginated.COL_DOC_NAME));
		}

		if (profileFileBrowser.isDocEffectiveDateVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.effectiveDate"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocEffectiveDateWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.effectiveDate"), String.valueOf(GWTPaginated.COL_DOC_EFFECTIVE_DATE));
		}

		if (profileFileBrowser.isDocOrgVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.org"));
			colDocOrgIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocOrgWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.org"), String.valueOf(GWTPaginated.COL_DOC_ORG));
		}

		if (profileFileBrowser.isDocConfidentialityVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.confidentiality.level"));
			colDocOrgIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocConfidentialityWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.confidentiality.level"), String.valueOf(GWTPaginated.COL_DOC_CONFIDENTIALITY));
		}

		if (profileFileBrowser.isDocExpiredDateVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.docExpiredDate"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocExpiredDateWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.docExpiredDate"), String.valueOf(GWTPaginated.COL_DOC_EXPIRED_DATE));
		}

		if (profileFileBrowser.isDocAuthorVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.docAuthor"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getDocAuthorWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.docAuthor"), String.valueOf(GWTPaginated.COL_DOC_AUTHOR));
		}

		if (profileFileBrowser.isPublishComVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.publishCom"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getPublishComWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.publishCom"), String.valueOf(GWTPaginated.COL_PUBLISH_COM));
		}

		if (profileFileBrowser.isPublishedYearVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.publishedYear"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getPublishedYearWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.publishedYear"), String.valueOf(GWTPaginated.COL_PUBLISHED_YEAR));
		}

		if (profileFileBrowser.isPageNumberVisible()) {
			headerTable.setHTML(row, col, Main.i18n("document.pageNumber"));
			colDocNameIndex = col;
			ScrollTableHelper.setColumnWidth(table, col, Integer.parseInt(profileFileBrowser.getPageNumberWidth()), ScrollTableHelper.MEDIUM, true, false);
			col++;
			fBController.addOrderByItem(Main.i18n("document.pageNumber"), String.valueOf(GWTPaginated.COL_PAGE_NUMBER));
		}

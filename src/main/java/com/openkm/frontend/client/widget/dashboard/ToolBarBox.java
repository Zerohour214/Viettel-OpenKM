...
public class ToolBarBox extends HorizontalPanel implements HasClickHandlers, HasAllMouseHandlers {
	 ...
	 * @param text
	 */
	public ToolBarBox(Widget glyphicon, String text) {
		super();
		...
		html.setTitle(text);
		icon.setTitle(text);
		icon.getElement().setAttribute("style","font-size: 40px;");
		...
		vPanel.setCellHorizontalAlignment(icon, HasAlignment.ALIGN_CENTER);

		setCellVerticalAlignment(vPanel, HasAlignment.ALIGN_MIDDLE);
		icon.getElement().setAttribute("style","padding-bottom: 4px;");

...
public class SearchControl extends Composite {
	...
	public ValueChangeHandler changeHandler;
	...
	public SearchControl() {
		...
		changeHandler = new ValueChangeHandler() {
		...
		cleanButton = new Button(OkmConstants.ICON_CLEAN_BUTTON + Main.i18n("button.clean"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				clean();
			}
		...
		saveSearchButton = new Button(OkmConstants.ICON_SAVE_BUTTON + Main.i18n("button.save.search"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				long domain = 0;
				...
				params.setDocCode(searchNormal.docCode.getText());
				params.setDocName(searchNormal.docName.getText());
				params.setDocEffectiveDate(searchNormal.docEffectDate.getValue());
				if (!searchNormal.orgListBox.getSelectedValue().isEmpty()) {
					params.setDocOrgId(Long.parseLong(searchNormal.orgListBox.getSelectedValue()));
				}
				else {
					params.setDocOrgId(0);
				}
				if (!searchNormal.docConfidentialityListBox.getSelectedValue().isEmpty()) {
					params.setDocConfidentiality(Integer.parseInt(searchNormal.docConfidentialityListBox.getSelectedValue()));
				}
				else {
					params.setDocConfidentiality(0);
				}
				params.setDocExpiredDate(searchNormal.docExpiredDate.getValue());
				params.setDocAuthor(searchNormal.docAuthor.getText());
				params.setPublishCom(searchNormal.publishCom.getText());
				if(!searchNormal.publishedYear.getText().isEmpty() && !searchNormal.publishedYear.getText().equals("")){
					params.setPublishedYear(Integer.valueOf(searchNormal.publishedYear.getText()));
				}
				if(!searchNormal.pageNumber.getText().isEmpty() && !searchNormal.pageNumber.getText().equals("")){
					params.setPageNumber(Integer.valueOf(searchNormal.pageNumber.getText()));
				}
		...
		advancedView.addStyleName("okm-checkbox-search-renew");
		compactResultsView.addStyleName("okm-checkbox-search-renew");
		showPropertyGroups.addStyleName("okm-checkbox-search-renew");
		saveUserNews.addStyleName("okm-checkbox-search-renew");
		searchButton.setStyleName("okm-SearchButton");
		searchButton.addStyleName("btn");
		searchButton.addStyleName("btn-primary");
		saveSearchButton.setStyleName("okm-SaveButton");
		saveSearchButton.addStyleName("btn");
		saveSearchButton.addStyleName("btn-success");
		saveSearchButton.addStyleName("okm-NoWrap");
		cleanButton.setStyleName("okm-CleanButton");
		cleanButton.addStyleName("btn");
		cleanButton.addStyleName("btn-danger");
	...
	public void executeSearch() {
		switch (searchMode) {
			...
			case SEARCH_MODE_ADVANCED:
				...
				gwtParams.setDocCode(searchNormal.docCode.getText());
				gwtParams.setDocName(searchNormal.docName.getText());
				gwtParams.setDocEffectiveDate(searchNormal.docEffectDate.getValue());
				gwtParams.setDocExpiredDate(searchNormal.docExpiredDate.getValue());
				gwtParams.setDocAuthor(searchNormal.docAuthor.getText());
				gwtParams.setPublishCom(searchNormal.publishCom.getText());
				if(!searchNormal.publishedYear.getText().isEmpty()){
					gwtParams.setPublishedYear(Integer.parseInt(searchNormal.publishedYear.getText()));
				}
				if(!searchNormal.pageNumber.getText().isEmpty()){
					gwtParams.setPageNumber(Integer.parseInt(searchNormal.pageNumber.getText()));
				}
				gwtParams.setAuthor(searchNormal.userListBox.getValue(searchNormal.userListBox.getSelectedIndex()));
				if (!searchNormal.orgListBox.getSelectedValue().isEmpty()) {
					gwtParams.setDocOrgId(Long.parseLong(searchNormal.orgListBox.getSelectedValue()));
				}
				else {
					gwtParams.setDocOrgId(0);
				}
				if (!searchNormal.docConfidentialityListBox.getSelectedValue().isEmpty()) {
					gwtParams.setDocConfidentiality(Integer.parseInt(searchNormal.docConfidentialityListBox.getSelectedValue()));
				}
				else {
					gwtParams.setDocConfidentiality(0);
				}
		...
		switch (searchMode) {
			...
			case SEARCH_MODE_ADVANCED:
				...
				if (searchNormal.content.getText().length() >= minSearchCharacters || searchNormal.name.getText().length() >= minSearchCharacters ||
						searchNormal.docCode.getText().length() >= minSearchCharacters || searchNormal.docName.getText().length() >= minSearchCharacters ||
						searchNormal.keywords.getText().length() >= minSearchCharacters || searchNormal.docAuthor.getText().length() >= minSearchCharacters ||
						searchNormal.publishCom.getText().length() >= minSearchCharacters || searchNormal.publishedYear.getText().length() >= minSearchCharacters ||
						searchNormal.pageNumber.getText().length() >= minSearchCharacters ||
						searchAdvanced.from.getText().length() >= minSearchCharacters || searchAdvanced.to.getText().length() >= minSearchCharacters ||
						searchAdvanced.subject.getText().length() >= minSearchCharacters) {
					searchButton.setEnabled(true);
				} else {
					searchButton.setEnabled(false);
				}
				if (searchNormal.docExpiredDate.getValue() != null || searchNormal.docEffectDate.getValue() != null ||
						searchNormal.orgListBox.getSelectedIndex() > 0 || searchNormal.docConfidentialityListBox.getSelectedIndex() > 0) {
					searchButton.setEnabled(true);
				}
	...
	public void langRefresh() {
		...
		cleanButton.setHTML(OkmConstants.ICON_CLEAN_BUTTON + Main.i18n("button.clean"));
		saveSearchButton.setHTML(OkmConstants.ICON_SAVE_BUTTON + Main.i18n("button.save.search"));
	...
	private void clean() {
		...
		searchNormal.docCode.setText("");
		searchNormal.docName.setText("");
		searchNormal.docEffectDate.setValue(null);
		searchNormal.orgListBox.setSelectedIndex(0);
		searchNormal.docConfidentialityListBox.setSelectedIndex(0);
		searchNormal.docExpiredDate.setValue(null);
		searchNormal.docAuthor.setText("");
		searchNormal.publishCom.setText("");
		searchNormal.publishedYear.setText("");
		searchNormal.publishedYear.setValue("");
		searchNormal.pageNumber.setText("");
		searchNormal.pageNumber.setValue("");

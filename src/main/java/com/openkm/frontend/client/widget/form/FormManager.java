	...
	private void drawFormElement(int row, final GWTFormElement gwtFormElement, boolean readOnly, boolean searchView) {
		...
		if (gwtFormElement instanceof GWTButton) {
			...
			if (((GWTInput) gwtFormElement).getType().equals(GWTInput.TYPE_DATE)) {
				...
				final HTML calendarIcon = new HTML("<span class=\"glyphicons glyphicons-calendar icon-green-renew\"></span>");

				if (readOnly || ((GWTInput) gwtFormElement).isReadonly()) {                        // read only
					calendarIcon.setHTML("<span class=\"glyphicons glyphicons-calendar icon-disable-renew\"></span>");
				} else {
				...
				if (!value.equals("")) {
					...
					hLinkPanel.add(new ClipboardIcon(url));
			...
			if (searchView || isMassiveView) {
				if (searchView) {
					...
					if (((GWTInput) gwtFormElement).getType().equals(GWTInput.TYPE_DATE)) {
						...
						// Clean
						final HTML cleanIcon = new HTML("<span class=\"glyphicons glyphicons-remove-sign icon-red-renew\"></span>");
			...
			if (gwtSelect.getType().equals(GWTSelect.TYPE_SIMPLE)) {
				...
				Button addButton = new Button(OkmConstants.ICON_ADD_BUTTON + Main.i18n("button.add"), new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {

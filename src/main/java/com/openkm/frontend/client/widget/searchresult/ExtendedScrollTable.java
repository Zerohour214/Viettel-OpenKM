		...
		if (profileFileBrowser.isIconVisible()) {
			...
			if ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
				if (folder.isHasChildren()) {
					dataTable.setHTML(rows, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-green-renew"));
				} else {
					dataTable.setHTML(rows, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-green-renew"));
				}
			} else {
				if (folder.isHasChildren()) {
					dataTable.setHTML(rows, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-red-renew"));
				} else {
					dataTable.setHTML(rows, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-red-renew"));
				}
			}
			dataTable.getCellFormatter().setHorizontalAlignment(rows, col++, HasHorizontalAlignment.ALIGN_RIGHT);
		}

...
public class ExtendedColumnSorter extends ColumnSorter {
		...
		if (correctedColumn <= 17) {
			for (int i = 0; i < rows; i++) {
				...
				switch (correctedColumn) {
					case UIDesktopConstants.FILEBROWSER_COLUMN_STATUS:
					case UIDesktopConstants.FILEBROWSER_COLUMN_MASSIVE:
					case UIDesktopConstants.FILEBROWSER_COLUMN_ICON:
					case UIDesktopConstants.FILEBROWSER_COLUMN_NAME:
					case UIDesktopConstants.FILEBROWSER_COLUMN_AUTHOR:
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_CODE:
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_AUTHOR:
					case UIDesktopConstants.FILEBROWSER_COLUMN_PUBLISH_COM:
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_NAME:
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_ORG:
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_CONFIDENTIALITY:
						// Text
						...
						break;
					case UIDesktopConstants.FILEBROWSER_COLUMN_PUBLISHED_YEAR:
					case UIDesktopConstants.FILEBROWSER_COLUMN_PAGE_NUMBER:
						rowToOrder.setObject(String.valueOf(rowI[column]).toLowerCase());        // Lower case solves problem with sort ordering
						rowToOrder.setDataId("" + i);                            // Actual position value
						elementToOrder.add(rowToOrder);
						break;

					case UIDesktopConstants.FILEBROWSER_COLUMN_SIZE:
						// Bytes
					...
					case UIDesktopConstants.FILEBROWSER_COLUMN_LASTMODIFIED:
						// Date
						if (data.get(Integer.parseInt(rowI[colDataIndex])) instanceof GWTFolder) {
					...
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_EFFECTIVE_DATE:
						// Date
						if (data.get(Integer.parseInt(rowI[colDataIndex])) instanceof GWTFolder) {
							rowToOrder.setObject(((GWTFolder) data.get(Integer.parseInt(rowI[colDataIndex]))).getCreated());    // Date value
							rowToOrder.setDataId("" + i);                                                            // Actual position value
							...
							if (((GWTDocument) data.get(Integer.parseInt(rowI[colDataIndex]))).getDocEffectiveDate() == null) {
								rowToOrder.setObject(new Date(0)); // Date value
							} else {
								rowToOrder.setObject(((GWTDocument) data.get(Integer.parseInt(rowI[colDataIndex]))).getDocEffectiveDate()); // Date value
							}
							rowToOrder.setDataId("" + i);                                                                 // Actual position value
							elementToOrder.add(rowToOrder);
						}
						break;
					case UIDesktopConstants.FILEBROWSER_COLUMN_DOC_EXPIRED_DATE:
						// Date
						if (data.get(Integer.parseInt(rowI[colDataIndex])) instanceof GWTFolder) {
							rowToOrder.setObject(((GWTFolder) data.get(Integer.parseInt(rowI[colDataIndex]))).getCreated());    // Date value
							rowToOrder.setDataId("" + i);                                                            // Actual position value
							...
							if (((GWTDocument) data.get(Integer.parseInt(rowI[colDataIndex]))).getDocExpiredDate() == null) {
								rowToOrder.setObject(new Date(0));
							}
							else {
								rowToOrder.setObject(((GWTDocument) data.get(Integer.parseInt(rowI[colDataIndex]))).getDocExpiredDate());
							}
							rowToOrder.setDataId("" + i);                                                                 // Actual position value
							elementToOrder.add(rowToOrder);
						}
					...
					case UIDesktopConstants.FILEBROWSER_COLUMN_VERSION:
						// Version
						if (data.get(Integer.parseInt(rowI[colDataIndex])) instanceof GWTFolder ||
					...
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA0:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA1:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA2:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA3:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA4:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA5:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA6:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA7:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA8:
					case UIDesktopConstants.FILEBROWSER_COLUMN_EXTRA9:
						if (data.get(Integer.parseInt(rowI[colDataIndex])) instanceof GWTFolder) {
	...
	private int correctedColumnIndex(int col) {
		...
		if (!profileFileBrowser.isDocCodeVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_CODE) {
			corrected++;
		}
		if (!profileFileBrowser.isDocNameVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_NAME) {
			corrected++;
		}
		if (!profileFileBrowser.isDocEffectiveDateVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_EFFECTIVE_DATE) {
			corrected++;
		}
		if (!profileFileBrowser.isDocOrgVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_ORG) {
			corrected++;
		}
		if (!profileFileBrowser.isDocConfidentialityVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_CONFIDENTIALITY) {
			corrected++;
		}
		if (!profileFileBrowser.isDocExpiredDateVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_EXPIRED_DATE) {
			corrected++;
		}
		if (!profileFileBrowser.isDocAuthorVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_DOC_AUTHOR) {
			corrected++;
		}
		if (!profileFileBrowser.isPublishComVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_PUBLISH_COM) {
			corrected++;
		}
		if (!profileFileBrowser.isPublishedYearVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_PUBLISHED_YEAR) {
			corrected++;
		}
		if (!profileFileBrowser.isPageNumberVisible() && corrected >= UIDesktopConstants.FILEBROWSER_COLUMN_PAGE_NUMBER) {
			corrected++;
		}

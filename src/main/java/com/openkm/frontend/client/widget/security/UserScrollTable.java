...
public class UserScrollTable extends Composite {
	...
	public void initExtendedSecurity(int extendedSecurity) {
		...
		if (isAssigned) {
			...
			if (evaluateGroup) {
				headerTable.setHTML(0, col, Main.i18n("security.user.permission.group"));
				ScrollTableHelper.setColumnWidth(table, col, 90, ScrollTableHelper.MEDIUM, false, true);
				col++;
			...
			if (evaluateHistory) {
				headerTable.setHTML(0, col, Main.i18n("security.user.permission.history"));
				ScrollTableHelper.setColumnWidth(table, col, 90, ScrollTableHelper.MEDIUM, false, true);
				col++;
				width += 55;
			}
		...
		if (evaluateGroup) {
			...
			ClickHandler checkGroupListener = new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					// it has inverse value
					if (((CheckBox) sender).getValue()) {
						grant(user.getId(), GWTPermission.PROPERTY_GROUP, Main.get().securityPopup.recursive.getValue());
					} else {
						revoke(user.getId(), GWTPermission.PROPERTY_GROUP, Main.get().securityPopup.recursive.getValue());
					}
				}
			};
		...
		if (evaluateHistory) {
			...
			ClickHandler checkHistoryListener = new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					// it has inverse value
					if (((CheckBox) sender).getValue()) {
						grant(user.getId(), GWTPermission.COMPACT_HISTORY, Main.get().securityPopup.recursive.getValue());
					} else {
						revoke(user.getId(), GWTPermission.COMPACT_HISTORY, Main.get().securityPopup.recursive.getValue());
					}
				}
			};
		...
		if (evaluateWorkflow) {
			...
			ClickHandler checkWorkflowListener = new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					// it has inverse value
					if (((CheckBox) sender).getValue()) {
						grant(user.getId(), GWTPermission.START_WORKFLOW, Main.get().securityPopup.recursive.getValue());
					} else {
						revoke(user.getId(), GWTPermission.START_WORKFLOW, Main.get().securityPopup.recursive.getValue());
					}
				}
			};
		...
		if (evaluateDownload) {
			...
			ClickHandler checkDownloadListener = new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					// it has inverse value
					if (((CheckBox) sender).getValue()) {
						grant(user.getId(), GWTPermission.DOWNLOAD, Main.get().securityPopup.recursive.getValue());
					} else {
						revoke(user.getId(), GWTPermission.DOWNLOAD, Main.get().securityPopup.recursive.getValue());
					}
				}
			};

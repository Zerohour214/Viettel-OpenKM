...
//					updateKeyword();
	...
	public void addPendingKeyWordsList() {
		if (!keyWordsListPending.isEmpty()) {
			...
			while (!keyWordsListPending.isEmpty()){
				listKeys.add(keyWordsListPending.remove(0));
			}
	...
	final AsyncCallback<Object> callbackAddListKeywords = new AsyncCallback<Object>() {
		public void onSuccess(Object result) {
			List<String> listKey = (List<String>) result;
			for(String keyword : listKey){
				Widget keywordButton = getKeyWidget(keyword, remove);
				keywordMap.put(keyword, keywordButton);
				hKeyPanel.add(keywordButton);
				keywords.add(keyword);
	...
	final AsyncCallback<Object> callbackUpdateKeywords = new AsyncCallback<Object>() {
		@Override
	...
	public void addListKeywords(List<String> listKeywords) {
...
//			}
		...
		propertyService.addListKeywords(path, listKeywords, callbackAddListKeywords);
	}
	...
	public void updateKeyword(){
		propertyService.updateKeyword(path, callbackUpdateKeywords);
	}

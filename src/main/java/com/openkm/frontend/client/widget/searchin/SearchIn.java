...
public class SearchIn extends Composite implements HasPropertyHandler {
	...
	public SearchIn() {
		...
		searchNormal.docCode.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.docName.addKeyUpHandler(searchControl.keyUpHandler);
		// add event for Datebox
		searchNormal.docEffectDate.addValueChangeHandler(searchControl.changeHandler);
		searchNormal.docExpiredDate.addValueChangeHandler(searchControl.changeHandler);
		searchNormal.docAuthor.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.publishCom.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.publishedYear.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.pageNumber.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.keywords.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.orgListBox.addKeyUpHandler(searchControl.keyUpHandler);
		searchNormal.docConfidentialityListBox.addKeyUpHandler(searchControl.keyUpHandler);
		...
		searchControl.addStyleName("searchIn-searchControl-renew");
		// Gets all users
		searchNormal.getAllUsers();

		searchNormal.getAllOrg();
		searchNormal.getConfidentialityLevels();
	...
	public void setPixelSize(int width, int height) {
		...
		searchSimple.setPixelSize(tabWidth - 2, height - TAB_HEIGHT); // Substract tab height
		searchNormal.setPixelSize(tabWidth - 2, height - TAB_HEIGHT); // Substract tab height
		searchAdvanced.setPixelSize(tabWidth - 2, height - TAB_HEIGHT); // Substract tab height
		searchMetadata.setPixelSize(tabWidth - 2, height - TAB_HEIGHT); // Substract tab height
	...
	public void setSavedSearch(GWTQueryParams gWTParams) {
		...
		searchNormal.docCode.setText(gWTParams.getDocCode());
		searchNormal.docName.setText(gWTParams.getDocName());
		searchNormal.docEffectDate.setValue(gWTParams.getDocEffectiveDate());
		searchNormal.docExpiredDate.setValue(gWTParams.getDocExpiredDate());
		searchNormal.docAuthor.setText(gWTParams.getDocAuthor());
		searchNormal.publishCom.setText(gWTParams.getPublishCom());
		searchNormal.publishedYear.setText(String.valueOf(gWTParams.getPublishedYear()));
		searchNormal.pageNumber.setText(String.valueOf(gWTParams.getPageNumber()));
		...
		if (gWTParams.getDocOrgId() > 0) {
			for (int i = 0; i < searchNormal.orgListBox.getItemCount(); i++) {
				if (searchNormal.orgListBox.getValue(i).equals(gWTParams.getDocOrgId())) {
					searchNormal.orgListBox.setSelectedIndex(i);
				}
			}
		}
		...
		if (gWTParams.getDocConfidentiality() > 0) {
			for (int i = 0; i < searchNormal.docConfidentialityListBox.getItemCount(); i++) {
				if (searchNormal.docConfidentialityListBox.getValue(i).equals(gWTParams.getDocConfidentiality())) {
					searchNormal.docConfidentialityListBox.setSelectedIndex(i);
				}
			}
		}

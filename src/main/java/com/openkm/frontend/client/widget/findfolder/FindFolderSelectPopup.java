...
public class FindFolderSelectPopup extends DialogBox {
	...
	public FindFolderSelectPopup() {
		...
		actionButton = new Button(OkmConstants.ICON_YES_BUTTON + Main.i18n("search.result.menu.go.folder"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
		...
		folderTable.addStyleName("okm-doc-search-result-renew");
	...
	public void langRefresh() {
		...
		actionButton.setHTML(OkmConstants.ICON_YES_BUTTON + Main.i18n("search.result.menu.go.folder"));
	}
				...
				if (gwtQueryResult.getFolder() != null) {
					...
					if ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
						if (folder.isHasChildren()) {
							folderTable.setHTML(rows, 0, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-green-renew"));
						} else {
							folderTable.setHTML(rows, 0, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-green-renew"));
						}
					} else {
						if (folder.isHasChildren()) {
							folderTable.setHTML(rows, 0, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-red-renew"));
						} else {
							folderTable.setHTML(rows, 0, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-red-renew"));
						}
					}

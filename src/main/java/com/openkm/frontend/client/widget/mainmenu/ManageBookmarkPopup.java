...
public class ManageBookmarkPopup extends DialogBox {
	...
	public ManageBookmarkPopup() {
		...
		updateButton = new Button(OkmConstants.ICON_EDIT_BUTTON + Main.i18n("button.update"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				if (selectedRow >= 0) {
		...
		updateButton.addStyleName("btn");
		updateButton.addStyleName("btn-success");
		updateButton.setEnabled(false);
	...
	final AsyncCallback<List<GWTBookmark>> callbackGetAll = new AsyncCallback<List<GWTBookmark>>() {
		public void onSuccess(List<GWTBookmark> result) {
			...
			for (Iterator<GWTBookmark> it = result.iterator(); it.hasNext(); ) {
				...
				String icon = "";
				if (bookmark.getType().equals(Bookmark.BOOKMARK_DOCUMENT)) {
					icon = "glyphicons glyphicons-file icon-green-renew";
				} else if (bookmark.getType().equals(Bookmark.BOOKMARK_FOLDER)) {
					icon = "glyphicons glyphicons-folder-flag icon-green-renew";
				}

				table.setHTML(row, 0, Util.imageHTMLIcon(icon));
	...
	public void langRefresh() {
		...
		updateButton.setHTML(OkmConstants.ICON_EDIT_BUTTON + Main.i18n("button.update"));
	}
}

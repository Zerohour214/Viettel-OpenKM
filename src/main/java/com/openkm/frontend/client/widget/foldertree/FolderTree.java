...
public class FolderTree extends Composite implements OriginPanel {
	...
	public FolderTree() {
		...
		rootItem.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new", "root_schema", "top"));
	...
	final AsyncCallback<GWTFolder> callbackCreate = new AsyncCallback<GWTFolder>() {
		...
		public void onSuccess(GWTFolder result) {
			...
			newFolder.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open", folder.getName(),"top"));
	...
	public void evaluesFolderIcon(TreeItem item) {
		...
		if ((folderItem.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
			if (folderItem.isHasChildren()) {
				item.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-green-renew " + subscribed, folderItem
						.getName(), "top"));
			} else {
				item.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-green-renew " + subscribed, folderItem
						.getName(), "top"));
			}
		} else {
			if (folderItem.isHasChildren()) {
				item.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-red-renew " + subscribed, folderItem
						.getName(), "top"));
			} else {
				item.setHTML(Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-red-renew " + subscribed, folderItem
						.getName(), "top"));
			}
		}
	}

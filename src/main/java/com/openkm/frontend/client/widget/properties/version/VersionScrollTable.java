...
public class VersionScrollTable extends Composite implements ClickHandler {
	...
	public VersionScrollTable() {
		...
		purge = new Button(OkmConstants.ICON_COMPACT_BUTTON + Main.i18n("version.purge.document"), this);
		purge.setStyleName("okm-CompactButton");
		purge.addStyleName("btn");
		purge.addStyleName("btn-primary");
		...
		ScrollTableHelper.setColumnWidth(table, 3, 100, ScrollTableHelper.MEDIUM, true, false);
		ScrollTableHelper.setColumnWidth(table, 4, 60, ScrollTableHelper.MEDIUM);
		ScrollTableHelper.setColumnWidth(table, 5, 90, ScrollTableHelper.FIXED);
		ScrollTableHelper.setColumnWidth(table, 6, 140, ScrollTableHelper.FIXED);
		ScrollTableHelper.setColumnWidth(table, 7, 150, ScrollTableHelper.MEDIUM, true, false);

		table.setColumnSortable(5, false);
		table.setColumnSortable(6, false);
		...
		headerTable.setHTML(0, 3, Main.i18n("version.oldName"));
		headerTable.setHTML(0, 4, Main.i18n("version.size"));
		headerTable.setHTML(0, 5, "&nbsp;");
		headerTable.setWidget(0, 6, purge);
		headerTable.setHTML(0, 7, Main.i18n("version.comment"));

		headerTable.getCellFormatter().setHorizontalAlignment(0, 6, HasAlignment.ALIGN_CENTER);
		headerTable.getCellFormatter().setVerticalAlignment(0, 6, HasAlignment.ALIGN_MIDDLE);
	...
	public void langRefresh() {
		...
		headerTable.setHTML(0, 4, Main.i18n("version.size"));
		headerTable.setHTML(0, 7, Main.i18n("version.comment"));
		purge.setHTML(OkmConstants.ICON_COMPACT_BUTTON + Main.i18n("version.purge.document"));
		...
		if (!buttonView.isEmpty()) {
			for (Iterator<Button> it = buttonView.iterator(); it.hasNext(); ) {
				...
				button.setHTML(OkmConstants.ICON_VIEW_BUTTON + Main.i18n("button.view"));
			}
		}
		...
		if (!buttonRestore.isEmpty()) {
			for (Iterator<Button> it = buttonRestore.iterator(); it.hasNext(); ) {
				Button button = it.next();
				button.setHTML(OkmConstants.ICON_YES_BUTTON + Main.i18n("button.restore"));
			}
		}
	}
	...
	public void addRow(GWTVersion version) {
		...
		dataTable.setHTML(rows, 3, version.getOldName());
		dataTable.setHTML(rows, 4, Util.formatSize(version.getSize()));
		dataTable.setHTML(rows, 7, version.getComment());
		...
		if (version.isActual() && visibleButtons) {
			...
			Button restoreButton = new Button(OkmConstants.ICON_YES_BUTTON + Main.i18n("button.restore"), new ClickHandler() {
				@Override
			...
			restoreButton.addStyleName("btn");
			restoreButton.addStyleName("btn-success");
		}

		Button viewButton = new Button(OkmConstants.ICON_VIEW_BUTTON + Main.i18n("button.view"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				List<String> versions = Main.get().mainPanel.desktop.browser.tabMultiple.tabDocument.version.versions;
				String ver = (String) versions.get(rows);
		...
		viewButton.addStyleName("btn");
		viewButton.addStyleName("btn-primary");
	}

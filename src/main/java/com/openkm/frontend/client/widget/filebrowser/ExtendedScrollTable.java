...
public class ExtendedScrollTable extends ScrollTable implements OriginPanel {
	...
	public void addRow(GWTFolder folder, boolean update) {
		...
		if (profileFileBrowser.isStatusVisible()) {
			if (folder.isSubscribed()) {
				dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-message-plus icon-green-renew"));
			} else {
				dataTable.setHTML(row, col, "&nbsp;");
			}

			if (folder.isHasNotes()) {
				dataTable.setHTML(row, col, dataTable.getHTML(row, col) + Util.imageItemHTMLIcon("glyphicons glyphicons-note-empty icon-green-renew"));
			}
		...
		if (profileFileBrowser.isIconVisible()) {
			...
			if ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
				if (folder.isHasChildren()) {
					dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-green-renew"));
				} else {
					dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-green-renew"));
				}
			} else {
				if (folder.isHasChildren()) {
					dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-red-renew"));
				} else {
					dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-red-renew"));
				}
			}
	...
	public void addNoteIconToSelectedRow() {
		if (profileFileBrowser.isStatusVisible()) {
			dataTable.setHTML(selectedRow, colStatusIndex,
					dataTable.getHTML(selectedRow, colStatusIndex) + Util.imageItemHTMLIcon("glyphicons glyphicons-note-empty icon-green-renew"));
		}
	}
	...
	public void deleteNoteIconToSelectedRow() {
		if (profileFileBrowser.isStatusVisible()) {
			...
			if (htmlValue.indexOf("edit.gif") >= 0) {
				newHtmlIcons = Util.imageItemHTMLIcon("glyphicons glyphicons-pencil icon-green-renew");
			} else if (htmlValue.indexOf("lock.gif") > 0) {
				newHtmlIcons = Util.imageItemHTMLIcon("glyphicons glyphicons-lock icon-green-renew");
			} else {
				newHtmlIcons = "&nbsp;";
			}

			if (htmlValue.indexOf("subscribed.gif") >= 0) {
				newHtmlIcons += Util.imageItemHTMLIcon("glyphicons glyphicons-message-plus icon-green-renew");
			}
	...
	public void addRow(GWTDocument doc, boolean update) {
		...
		if (profileFileBrowser.isStatusVisible()) {
			if (doc.isCheckedOut()) {
				dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-pencil icon-green-renew"));
			} else if (doc.isLocked()) {
				dataTable.setHTML(row, col, Util.imageItemHTMLIcon("glyphicons glyphicons-lock icon-green-renew"));
			} else {
				dataTable.setHTML(row, col, "&nbsp;");
			}
			...
			if (doc.isSubscribed()) {
				dataTable
						.setHTML(row, col, dataTable.getHTML(row, col) + Util.imageItemHTMLIcon("glyphicons glyphicons-message-plus icon-green-renew"));
			}
		...
		if (profileFileBrowser.isDocCodeVisible()) {
			dataTable.setText(row, col, doc.getDocCode());
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}

		if (profileFileBrowser.isDocNameVisible()) {
			dataTable.setText(row, col, doc.getDocName());
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		...
		if (profileFileBrowser.isDocEffectiveDateVisible()) {
			...
			if (doc.getDocEffectiveDate() == null) {
				...
				dataTable.setHTML(row, col, dtf.format(doc.getDocEffectiveDate()));
			}
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}

		if (profileFileBrowser.isDocOrgVisible()) {
			dataTable.setHTML(row, col, doc.getDocOrgName());
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}

		if (profileFileBrowser.isDocConfidentialityVisible()) {
			dataTable.setText(row, col, Main.i18n(doc.getDocConfidentialityName()));
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		...
		if (profileFileBrowser.isDocExpiredDateVisible()) {
			...
			if (doc.getDocExpiredDate() == null) {
				...
				dataTable.setHTML(row, col, dtf.format(doc.getDocExpiredDate()));
			}
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		if (profileFileBrowser.isDocAuthorVisible()) {
			dataTable.setText(row, col, doc.getDocAuthor());
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		if (profileFileBrowser.isPublishComVisible()) {
			dataTable.setText(row, col, doc.getPublishCom());
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		if (profileFileBrowser.isPublishedYearVisible()) {
			dataTable.setHTML(row, col, String.valueOf(doc.getPublishYear()));
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
		if (profileFileBrowser.isPageNumberVisible()) {
			dataTable.setHTML(row, col, String.valueOf(doc.getPageNumber()));
			dataTable.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
		}
				...
				if (isDragged() && mouseDownX > 0 && mouseDownY > 0 && evalDragPixelSensibility()) {
					...
					if (isSelectedRow()
						...
						if (isDocumentSelected()) {
							...
							if ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
								if (folder.isHasChildren()) {
									dragText = Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-green-renew");
								} else {
									dragText = Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-green-renew");
								}
							} else {
								if (folder.isHasChildren()) {
									dragText = Util.imageItemHTMLIcon("glyphicons glyphicons-folder-new icon-red-renew");
								} else {
									dragText = Util.imageItemHTMLIcon("glyphicons glyphicons-folder-open icon-red-renew");
								}
							}

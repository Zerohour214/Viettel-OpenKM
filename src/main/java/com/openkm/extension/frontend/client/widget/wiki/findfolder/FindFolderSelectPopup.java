...
package com.openkm.extension.frontend.client.widget.wiki.findfolder;
...
public class FindFolderSelectPopup extends DialogBox {
	...
	public FindFolderSelectPopup() {
		...
		cancelButton = new Button(OkmConstants.ICON_NO_BUTTON + GeneralComunicator.i18n("button.close"), new ClickHandler() {
			@Override
		...
		actionButton = new Button(OkmConstants.ICON_YES_BUTTON + GeneralComunicator.i18nExtension("wiki.add.link"), new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				String fldPath = folderTable.getText(selectedRow, 1);
				Wiki.get().addFolderTag(folderTable.getText(selectedRow, 2), UtilComunicator.getName(fldPath));
				hide();
			}
		});
	...
	public void langRefresh() {
		...
		cancelButton.setHTML(OkmConstants.ICON_NO_BUTTON + GeneralComunicator.i18n("button.close"));
		actionButton.setHTML(OkmConstants.ICON_YES_BUTTON + GeneralComunicator.i18nExtension("wiki.add.link"));
	}
				...
				if (gwtQueryResult.getFolder() != null) {
					...
					if ((folder.getPermissions() & GWTPermission.WRITE) == GWTPermission.WRITE) {
						if (folder.isHasChildren()) {
							folderTable.setHTML(rows, 0, UtilComunicator.iconItemHTML("glyphicons glyphicons-folder-new icon-green-renew"));
						} else {
							folderTable.setHTML(rows, 0, UtilComunicator.iconItemHTML("glyphicons glyphicons-folder-open icon-green-renew"));
						}
					} else {
						if (folder.isHasChildren()) {
							folderTable.setHTML(rows, 0, UtilComunicator.iconItemHTML("glyphicons glyphicons-folder-new icon-red-renew"));
						} else {
							folderTable.setHTML(rows, 0, UtilComunicator.iconItemHTML("glyphicons glyphicons-folder-open icon-red-renew"));
						}
					}

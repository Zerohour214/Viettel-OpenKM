...
public class WikiHistory extends Composite {
	...
	public void showHistory(final List<GWTWikiPage> wikiPages, boolean locked) {
		...
		for (GWTWikiPage wikiPage : wikiPages) {
			...
			// Restore button
			if (!locked && row > 0) {
				Button restoreButton = new Button(OkmConstants.ICON_YES_BUTTON + GeneralComunicator.i18n("button.restore"));
			...
			if (row > 0) {
				Button showButton = new Button(OkmConstants.ICON_VIEW_BUTTON + GeneralComunicator.i18n("button.view"));
				...
				showButton.addStyleName("btn");
				showButton.addStyleName("btn-primary");

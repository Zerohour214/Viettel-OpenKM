...
public class ConfirmPopup extends DialogBox {
	...
	private void execute() {
		switch (action) {
			case CONFIRM_DELETE_WIKI_PAGE:
				controller.deleteWikiPage(wikiPage);
				break;
		}
	 ...
	 * @param controller
	 */
	public void setConfirm(int action, GWTWikiPage wikiPage, WikiController controller) {
		this.action = action;
		this.wikiPage = wikiPage;
		this.controller = controller;
		switch (action) {
			case CONFIRM_DELETE_WIKI_PAGE:
				text.setHTML(GeneralComunicator.i18nExtension("wiki.confirm.delete"));
				break;
		}
	}

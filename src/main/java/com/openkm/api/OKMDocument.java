...
public class OKMDocument implements DocumentModule {
	...
	@Override
	public Version checkin(String token, String docId, InputStream is, String comment, HashMap<String, Object> vtProperties)
		...
		DocumentModule dm = ModuleManager.getDocumentModule();
		Version version = dm.checkin(token, docId, is, comment, vtProperties);
		log.debug("checkin: {}", version);
		return version;
	}

	@Override
	public Version checkin(String token, String docId, InputStream is, String comment, int increment, HashMap<String, Object> vtProperties)
			throws FileSizeExceededException, UserQuotaExceededException, VirusDetectedException, LockException,
			VersionException, PathNotFoundException, AccessDeniedException, RepositoryException, IOException,
			DatabaseException, ExtensionException, AutomationException {
		log.debug("checkin({}, {}, {}, {})", new Object[]{token, docId, comment, increment});
		DocumentModule dm = ModuleManager.getDocumentModule();
		Version version = dm.checkin(token, docId, is, comment, increment, vtProperties);
		log.debug("checkin: {}", version);
		return version;
	}

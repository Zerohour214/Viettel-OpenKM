				...
				switch (Integer.parseInt(key)) {
					...
					case GWTPaginated.COL_DOC_CODE:
						if (obj instanceof NodeFolder) {
							...
							String docCode = ((NodeDocument) obj).getDocCode();
							if (!docCode.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_DOC_NAME:
						if (obj instanceof NodeFolder) {
							...
							String docName = ((NodeDocument) obj).getDocName();
							if (!docName.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_DOC_AUTHOR:
						if (obj instanceof NodeFolder) {
							...
							String docAuthor = ((NodeDocument) obj).getDocAuthor();
							if (!docAuthor.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_PUBLISH_COM:
						if (obj instanceof NodeFolder) {
							...
							String publishCom = ((NodeDocument) obj).getPublishCom();
							if (!publishCom.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_PUBLISHED_YEAR:
						if (obj instanceof NodeFolder) {
							...
							Integer publishedYear = ((NodeDocument) obj).getPublishedYear();
							if (!publishedYear.equals(Integer.valueOf(mapFilter.get(key).getFilterValue1()))) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_PAGE_NUMBER:
						if (obj instanceof NodeFolder) {
							...
							Integer pageNumber = ((NodeDocument) obj).getPageNumber();
							if (!pageNumber.equals(Integer.valueOf(mapFilter.get(key).getFilterValue1()))) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_DOC_EFFECTIVE_DATE:
						if (obj instanceof NodeFolder) {
							...
							if (mapFilter.get(key).getFrom() != null
									&& ((NodeDocument) obj).getDocEffectiveDate().getTime()
									.compareTo(mapFilter.get(key).getFrom()) <= 0) {
								add = false;
							}
					...
					case GWTPaginated.COL_DOC_ORG:
						if (obj instanceof NodeFolder) {
							...
							String docOrgName = OrganizationDao.getInstance().getOrgNameById(((NodeDocument) obj).getDocOrgId());
							if (!docOrgName.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
					...
					case GWTPaginated.COL_DOC_EXPIRED_DATE:
						if (obj instanceof NodeFolder) {
							...
							if (mapFilter.get(key).getFrom() != null
									&& ((NodeDocument) obj).getDocExpiredDate().getTime()
									.compareTo(mapFilter.get(key).getFrom()) <= 0) {
								add = false;
							}
					...
					case GWTPaginated.COL_DOC_CONFIDENTIALITY:
						if (obj instanceof NodeFolder) {
							...
							String docConfidentiality = String.valueOf(((NodeDocument) obj).getDocConfidentiality());
							if (!docConfidentiality.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof NodeMail) {
							add = false;
				...
				switch (Integer.parseInt(key)) {
					...
					case GWTPaginated.COL_DOC_AUTHOR:
						if (obj instanceof GWTFolder) {
							...
							if (!((GWTDocument) obj).getDocAuthor()
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_PUBLISH_COM:
						if (obj instanceof GWTFolder) {
							...
							if (!((GWTDocument) obj).getPublishCom()
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_PUBLISHED_YEAR:
						if (obj instanceof GWTFolder) {
							...
							if (!String.valueOf(((GWTDocument) obj).getPublishYear())
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_PAGE_NUMBER:
						if (obj instanceof GWTFolder) {
							...
							if (!String.valueOf(((GWTDocument) obj).getPageNumber())
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_DOC_EFFECTIVE_DATE:
						if (obj instanceof GWTFolder) {
							...
							if (mapFilter.get(key).getFrom() != null
									&& ((GWTDocument) obj).getDocEffectiveDate()
									.compareTo(mapFilter.get(key).getFrom()) <= 0) {
								add = false;
							}
							if (mapFilter.get(key).getTo() != null
					...
					case GWTPaginated.COL_DOC_ORG:
						if (obj instanceof GWTFolder) {
							...
							if (!OrganizationDao.getInstance().getOrgNameById(((GWTDocument) obj).getDocOrgId())
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_DOC_CONFIDENTIALITY:
						if (obj instanceof GWTFolder) {
							...
							if (!String.valueOf(((GWTDocument) obj).getDocConfidentiality())
									.equals(mapFilter.get(key).getFilterValue1())) {
								add = false;
							}
						} else if (obj instanceof GWTMail) {
					...
					case GWTPaginated.COL_DOC_EXPIRED_DATE:
						if (obj instanceof GWTFolder) {
							...
							if (mapFilter.get(key).getFrom() != null
									&& ((GWTDocument) obj).getDocExpiredDate()
									.compareTo(mapFilter.get(key).getFrom()) <= 0) {
								add = false;
							}
							if (mapFilter.get(key).getTo() != null

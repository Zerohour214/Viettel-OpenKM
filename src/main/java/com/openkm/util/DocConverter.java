...
import viettel.filter.KmConnector;
...
public class DocConverter {
	...
	public void doc2pdf(File input, String mimeType, File output) throws ConversionException, DatabaseException, IOException {
		...
		try {
			...
			temp = new File("temp.pdf");
			convert(input, mimeType, temp);
			log.debug("Elapse doc2pdf time: {}", FormatUtil.formatSeconds(System.currentTimeMillis() - start));

//			 Adding watermark
			PdfReader reader = new PdfReader(temp.getAbsolutePath());

			log.info("---------------- BH ĐÃ QUA CONVERT ------------------------");
			document = new Document(new PdfReader(temp.getAbsolutePath()).getPageSize(1));
			...
			String watermark = userId + " " + userName + "  " + dtf.format(now);
			document.open();
			for(int i =1 ; i <= num; i++){
				document.newPage();
			...
			throw new ConversionException("Error in " + mimeType + " to PDF conversion " + e.getMessage(), e);
		} finally {
	...
	public static void doc2pdf(String token, String docId, String dstPath) throws RepositoryException, PathNotFoundException,
		...
		try {
			...
			try {
				...
				OKMDocument.getInstance().checkin(token, dstPath, docIs, "Document to PDF", new HashMap<>());
			}
		} finally {
			IOUtils.closeQuietly(docIs);
			IOUtils.closeQuietly(docOs);

...
public class GWTUtil {
	...
	public static GWTDocument copy(Document doc, GWTWorkspace workspace) throws PrincipalAdapterException, IOException,
		...
		gwtDocument.setDocCode(doc.getDocCode());
		gwtDocument.setDocName(doc.getDocName());
		if (doc.getDocEffectiveDate() != null) {
			gwtDocument.setDocEffectiveDate(doc.getDocEffectiveDate().getTime());
		}
		gwtDocument.setDocOrgId(doc.getDocOrgId());
		gwtDocument.setDocConfidentiality(doc.getDocConfidentiality());
		if (doc.getDocExpiredDate() != null) {
			gwtDocument.setDocExpiredDate(doc.getDocExpiredDate().getTime());
		}
		gwtDocument.setDocAuthor(doc.getDocAuthor());
		gwtDocument.setPublishCom(doc.getPublishCom());
		if(doc.getPublisedYear() != null){
			gwtDocument.setPublishYear(doc.getPublisedYear());
		}
		if(doc.getPageNumber() != null){
			gwtDocument.setPageNumber(doc.getPageNumber());
		}
	...
	public static GWTVersion copy(Version version) throws PrincipalAdapterException {
		...
		gWTVersion.setOldName(version.getOldName());
...
//				if(!kw[i].isEmpty() && !kw[i].equals(""))
//				tmpKwd.add(kw[i].trim().toLowerCase());
//			}
//		}
		...
		Calendar docEffectiveDate = Calendar.getInstance();
		Calendar docExpiredDate = Calendar.getInstance();

		if (gWTParams.getDocEffectiveDate() != null) {
			docEffectiveDate.setTime(gWTParams.getDocEffectiveDate());
		} else {
			docEffectiveDate = null;
		}

		if (gWTParams.getDocExpiredDate() != null) {
			docExpiredDate.setTime(gWTParams.getDocExpiredDate());
		} else {
			docExpiredDate = null;
		}
		...
		params.setDocCode(gWTParams.getDocCode());
		params.setDocName(gWTParams.getDocName());
		params.setDocEffectiveDate(docEffectiveDate);
		params.setDocOrgId(gWTParams.getDocOrgId());
		params.setDocConfidentiality(gWTParams.getDocConfidentiality());
		params.setDocExpiredDate(docExpiredDate);
		params.setDocAuthor(gWTParams.getDocAuthor());
		params.setPublishCom(gWTParams.getPublishCom());
		params.setPublishedYear(gWTParams.getPublishedYear());
		params.setPageNumber(gWTParams.getPageNumber());
		return params;
	}
	...
	public static GWTQueryParams copy(QueryParams params) throws RepositoryException, IOException, AccessDeniedException,
		...
		gWTParams.setDocName(params.getDocName());
		gWTParams.setDocCode(params.getDocCode());
		gWTParams.setDocAuthor(params.getDocAuthor());
		gWTParams.setPublishCom(params.getPublishCom());
		if (itCat.hasNext()) {

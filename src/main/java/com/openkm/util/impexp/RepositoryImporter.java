...
public class RepositoryImporter {
	...
	public static ImpExpStats importDocuments(String token, File fs, String fldPath, boolean metadata, boolean history,
			boolean uuid, Writer out, InfoDecorator deco, HashMap<String, Object> vtProperties) throws PathNotFoundException, AccessDeniedException,
		...
		try {
			...
			if (fs.exists()) {
				stats = importDocumentsHelper(token, fs, fldPath, metadata, history, uuid, out, deco, vtProperties);
			} else {
	...
	private static ImpExpStats importDocumentsHelper(String token, File fs, String fldPath, boolean metadata, boolean history,
			boolean uuid, Writer out, InfoDecorator deco, HashMap<String, Object> vtProperties) throws PathNotFoundException, AccessDeniedException, RepositoryException,
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (!fileName.endsWith(Config.EXPORT_METADATA_EXT)) {
				if (files[i].isDirectory()) {
					...
					ImpExpStats tmp = importDocumentsHelper(token, files[i], fld.getPath(), metadata, history, uuid, out, deco, vtProperties);
					...
					if (fileName.endsWith(".eml") || fileName.endsWith(".msg")) {
						...
						ImpExpStats tmp = importDocument(token, fs, fldPath, fileName, files[i], metadata, history, uuid, out, deco, vtProperties);
	...
	private static ImpExpStats importDocument(String token, File fs, String fldPath, String fileName, File fDoc,
			boolean metadata, boolean history, boolean uuid, Writer out, InfoDecorator deco, HashMap<String, Object> vtProperties) throws IOException,
				...
				if (history) {
					...
					for (File vhf : vhFiles) {
						...
						if (first) {
							...
							dm.checkin(token, doc.getPath(), fis, "Imported from administration", new HashMap<>());
						}

...
public class LogCatServlet extends BaseServlet {
	...
	private void view(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		String conPath = lf.getCanonicalPath();
		String absPath = lf.getAbsolutePath();
		String path = lf.getPath();
		if(conPath.equals(absPath) && path.equals(conPath) && "log".equals(Files.getFileExtension(lf.getName())) ){
			...
			sc.setAttribute("messages", "Tên file không hợp lệ!");
			sc.getRequestDispatcher("/admin/logcat.jsp").forward(request, response);
		}

...
public class PropertyServlet extends OKMRemoteServiceServlet implements OKMPropertyService {
	...
	public String addKeyword(String nodePath, String keyword) throws OKMException {
		...
		System.out.println("ADD KEYWORD " + keyword);
		String ret = null;
		updateSessionManager();
	...
	@Override
	public List<String> addListKeywords(String nodePath, List<String> keywords) throws OKMException {
		log.debug("addListKeywords({}, {})", nodePath, keywords);
		System.out.println("ADD LIST KEYWORD " + keywords);
		List<String> ret = null;
		updateSessionManager();

		try {
			ret = OKMProperty.getInstance().addListKeyword(null, nodePath, keywords);
		} catch (VersionException e) {
			log.error(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMPropertyService, ErrorCode.CAUSE_Version), e.getMessage());
		} catch (LockException e) {
	...
	@Override
	public void updateKeyword(String nodePath) throws OKMException {
		log.debug("updateKeyword({})", nodePath);
		System.out.println("UPDATE KEYWORD " + nodePath);
		updateSessionManager();
		try {
			OKMProperty.getInstance().updateKeyword(null, nodePath);
		} catch (VersionException e) {
			log.error(e.getMessage(), e);
			throw new OKMException(ErrorCode.get(ErrorCode.ORIGIN_OKMPropertyService, ErrorCode.CAUSE_Version), e.getMessage());
		} catch (LockException e) {

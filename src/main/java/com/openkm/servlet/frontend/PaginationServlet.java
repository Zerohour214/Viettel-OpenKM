	...
	public GWTPaginated getChildrenPaginated(String fldPath, boolean extraColumns, int offset, int limit, int order,
		...
		try {
			...
			if (fldPath.startsWith("/" + Repository.CATEGORIES)) {
				...
				totalDocuments = colDocuments.size();
			} else if (fldPath.startsWith("/" + Repository.THESAURUS)) {
				String keyword = fldPath.substring(fldPath.lastIndexOf("/") + 1).replace(" ", "_");
				...
				totalDocuments = NodeDocumentDAO.getInstance().countDocumentChildTotal(fldUuid);

				int newOffset = 0;

				if(offset >= colFolders.size() + totalDocuments){
					if (colFolders.size() + totalDocuments == 0 || colFolders.size() + totalDocuments <= limit) {
						newOffset = 0;
					} else if ((colFolders.size() + totalDocuments) % limit != 0) { // case there's some remainder
						newOffset = ((colFolders.size() + totalDocuments) / limit) * limit - colFolders.size();
					} else { // case exact division
						newOffset = ((colFolders.size() + totalDocuments) / limit) * limit;
						newOffset = offset - limit - colFolders.size(); // back limit to see latest values
					}
				} else {
					newOffset = offset - colFolders.size();
				}

				colDocuments = NodeDocumentDAO.getInstance().findByParentWithOffset(fldUuid, newOffset, limit);
			}



//			paginated.setTotalDocuments(colDocuments.size());
			paginated.setTotalDocuments(totalDocuments);
			...
			int finalTotal = totalDocuments + total - colDocuments.size();

			paginated.setTotal(finalTotal);

			// Testing offset error ( trying to find correct offset ).
			if (offset >= finalTotal) {
				if (finalTotal == 0 || finalTotal <= limit) {
					offset = 0;
				} else if (finalTotal % limit != 0) { // case there's some remainder
					offset = (finalTotal / limit) * limit;
				} else { // case exact division
					offset = (finalTotal / limit) * limit;
			...
			switch (order) {
				...
				case GWTPaginated.COL_DOC_CODE:
					Collections.sort(convertedCol, OrderByDocCode.getInstance());
					break;

				case GWTPaginated.COL_DOC_NAME:
					Collections.sort(convertedCol, OrderByDocName.getInstance());
					break;

				case GWTPaginated.COL_DOC_EFFECTIVE_DATE:
					Collections.sort(convertedCol, OrderByDocEffectiveDate.getInstance());
					break;

				case GWTPaginated.COL_DOC_ORG:
					Collections.sort(convertedCol, OrderByDocOrg.getInstance());

				case GWTPaginated.COL_DOC_EXPIRED_DATE:
					Collections.sort(convertedCol, OrderByDocExpiredDate.getInstance());
					break;

				case GWTPaginated.COL_DOC_AUTHOR:
					Collections.sort(convertedCol, OrderByDocAuthor.getInstance());
					break;

				case GWTPaginated.COL_PUBLISH_COM:
					Collections.sort(convertedCol, OrderByDocPublishCom.getInstance());
					break;

				case GWTPaginated.COL_PUBLISHED_YEAR:
					Collections.sort(convertedCol, OrderByPublishedYear.getInstance());
					break;

				case GWTPaginated.COL_PAGE_NUMBER:
					Collections.sort(convertedCol, OrderByPageNumber.getInstance());
					break;

				case GWTPaginated.COL_DOC_CONFIDENTIALITY:
					Collections.sort(convertedCol, OrderByDocConfidentiality.getInstance());
					break;
			...
			if (actual <= offset) {
				boolean found = false;
				if(offset <= colFolders.size() && colFolders.size() - offset <= limit){
					found = (actual == offset);
				} else  {
					offset = colFolders.size();
					found = (actual == offset);
				}
	...
	private static class OrderByPublishedYear implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByPublishedYear();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			if (arg0.getObj() instanceof NodeDocument && arg1.getObj() instanceof NodeDocument) {
				try {
					...
					int publishedYear =  ((NodeDocument) arg0.getObj()).getPublishedYear();
					String value0 = String.valueOf(publishedYear);

					int publishedYear1 = ((NodeDocument) arg1.getObj()).getPublishedYear();
					String value1 = String.valueOf(publishedYear1);
	...
	private static class OrderByPageNumber implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByPageNumber();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			if (arg0.getObj() instanceof NodeDocument && arg1.getObj() instanceof NodeDocument) {
				try {
					...
					int pageNumber =  ((NodeDocument) arg0.getObj()).getPageNumber();
					String value0 = String.valueOf(pageNumber);

					int pageNumber1 = ((NodeDocument) arg1.getObj()).getPageNumber();
					String value1 = String.valueOf(pageNumber1);
	 ...
	 *
	 */
	private static class OrderByDocCode implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocCode();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = ((NodeDocument) arg0.getObj()).getDocCode();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = ((NodeDocument) arg1.getObj()).getDocCode();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {
	...
	private static class OrderByDocAuthor implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocAuthor();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = ((NodeDocument) arg0.getObj()).getDocAuthor();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = ((NodeDocument) arg1.getObj()).getDocAuthor();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {
	...
	private static class OrderByDocPublishCom implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocPublishCom();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = ((NodeDocument) arg0.getObj()).getPublishCom();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = ((NodeDocument) arg1.getObj()).getPublishCom();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {
	 ...
	 *
	 */
	private static class OrderByDocName implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocName();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = ((NodeDocument) arg0.getObj()).getDocName();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = ((NodeDocument) arg1.getObj()).getDocName();
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {
	...
	/**
	 * OrderByDocEffectiveDate
	 *
	 * @author jllort
	 *
	 */
	private static class OrderByDocEffectiveDate implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocEffectiveDate();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				value0 = ((NodeDocument) arg0.getObj()).getDocEffectiveDate();
				if (value0 == null) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				value1 = ((NodeDocument) arg1.getObj()).getDocEffectiveDate();
				if (value1 == null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(new Date(0));
					value1 = cal;
	...
	private static class OrderByDocExpiredDate implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocExpiredDate();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				value0 = ((NodeDocument) arg0.getObj()).getDocExpiredDate();
				if (value0 == null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(new Date(0));
					value0 = cal;
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				value1 = ((NodeDocument) arg1.getObj()).getDocExpiredDate();
				if (value1 == null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(new Date(0));
					value1 = cal;
	 ...
	 *
	 */
	private static class OrderByDocOrg implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocName();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = OrganizationDao.getInstance().getOrgNameById(((NodeDocument) arg0.getObj()).getDocOrgId());
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = OrganizationDao.getInstance().getOrgNameById(((NodeDocument) arg1.getObj()).getDocOrgId());
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {
	...
	/**
	 * OrderByDocConfidentiality
	 *
	 * @author jllort
	 *
	 */
	private static class OrderByDocConfidentiality implements Comparator<ObjectToOrder> {
		private static final Comparator<ObjectToOrder> INSTANCE = new OrderByDocName();
		...
		public int compare(ObjectToOrder arg0, ObjectToOrder arg1) {
			...
			if (arg0.getObj() instanceof NodeFolder) {
				...
				try {
					value0 = String.valueOf(((NodeDocument) arg0.getObj()).getDocConfidentiality());
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg0.getObj() instanceof NodeMail) {
			...
			if (arg1.getObj() instanceof NodeFolder) {
				...
				try {
					value1 = String.valueOf(((NodeDocument) arg1.getObj()).getDocConfidentiality());
				} catch (Exception e) {
					// Ignore
				}
			} else if (arg1.getObj() instanceof NodeMail) {

...
import com.openkm.validator.email.EmailValidator;
...
public class WorkspaceServlet extends OKMRemoteServiceServlet implements OKMWorkspaceService {
	...
	public GWTWorkspace getUserWorkspace() throws OKMException {
		...
		try {
			...
			workspace.setTabDocumentRelatedVisible(up.getPrfTab().getPrfDocument().isRelatedVisible());
			workspace.setTabDocumentActivityVisible(up.getPrfTab().getPrfDocument().isActivityVisible());
	...
	@Override
	public String isValidForm(String password, String email) throws OKMException {
		log.debug("isValidForm()");
		String msg = "";
		updateSessionManager();

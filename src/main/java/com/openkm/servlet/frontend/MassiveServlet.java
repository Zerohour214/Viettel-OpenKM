...
public class MassiveServlet extends OKMRemoteServiceServlet implements OKMMassiveService {
	...
	@Override
	public void addListKeywords(List<String> paths, List<String> keywords) throws OKMException {
		log.debug("addListKeywords({},{})", paths, keywords);
		updateSessionManager();
		String error = "";
		String pathErrors = "";
		...
		for (String path : paths) {
			try {
//				OKMProperty.getInstance().addKeyword(null, path, keyword);
				OKMProperty.getInstance().addListKeyword(null, path, keywords);
			} catch (VersionException e) {
				log.error(e.getMessage(), e);
				error += "\n" + e.getMessage();
				pathErrors += "\n" + path;

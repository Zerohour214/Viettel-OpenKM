...
public class FileUploadServlet extends OKMHttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			i18n = LanguageUtils.getTranslations(lang, new String[]{Translation.MODULE_FRONTEND, Translation.MODULE_EXTENSION});
				...
				for (Iterator<FileItem> it = items.iterator(); it.hasNext(); ) {
					...
					if (item.isFormField()) {
						if (item.getFieldName().equals("path")) {
							...
							importZip = true;
						} else if (item.getFieldName().equals("docCode")) {
							vtProperties.put("docCode", item.getString("UTF-8"));
						} else if (item.getFieldName().equals("docName")) {
							vtProperties.put("docName", item.getString("UTF-8"));
						} else if (item.getFieldName().equals("docEffectiveDate")) {
							if (!item.getString("UTF-8").isEmpty()) {
								Calendar calendar = Calendar.getInstance();
								String formatDate = "dd-MM-yyyy";
								if (!i18n.isEmpty()){
									formatDate = i18n.get("frontend.general.day.pattern");
								}
								DateFormat dateFormat = new SimpleDateFormat(formatDate);
								calendar.setTime(dateFormat.parse(item.getString("UTF-8")));
								vtProperties.put("docEffectiveDate", calendar);
							}
						} else if (item.getFieldName().equals("docOrgId")) {
							if (!item.getString("UTF-8").isEmpty()) {
								vtProperties.put("docOrgId", item.getString("UTF-8"));
							}
						} else if (item.getFieldName().equals("docConfidentiality")) {
							vtProperties.put("docConfidentiality", item.getString("UTF-8"));
						} else  if (item.getFieldName().equals("docExpiredDate")) {
							if (!item.getString("UTF-8").isEmpty()) {
								Calendar calendar = Calendar.getInstance();
								String formatDate = "dd-MM-yyyy";
								if (!i18n.isEmpty()) {
								...
								DateFormat dateFormat = new SimpleDateFormat(formatDate);
								calendar.setTime(dateFormat.parse(item.getString("UTF-8")));
								vtProperties.put("docExpiredDate", calendar);
							}
						} else  if (item.getFieldName().equals("docAuthor")){
							vtProperties.put("docAuthor", item.getString("UTF-8"));
						} else if (item.getFieldName().equals("publishCom")){
							vtProperties.put("publishCom", item.getString("UTF-8"));
						} else if (item.getFieldName().equals("publishedYear")){
							if(!item.getString("UTF-8").isEmpty() || !item.getString("UTF-8").equals("")){
								vtProperties.put("publishedYear", Integer.valueOf(item.getString("UTF-8")));
							}
						}  else if (item.getFieldName().equals("pageNumber")){
							if(!item.getString("UTF-8").isEmpty() || !item.getString("UTF-8").equals("")){
								vtProperties.put("pageNumber", Integer.valueOf(item.getString("UTF-8")));
							}
				...
				if (action == UIFileUploadConstants.ACTION_INSERT) {
					if (fileName != null && !fileName.equals("")) {
						if (importZip && FilenameUtils.getExtension(fileName).equalsIgnoreCase("zip")) {
							...
							String erroMsg = importZip(path, is, vtProperties);
							...
							String erroMsg = importJar(path, is, vtProperties);
							...
							Document doc = new Document(vtProperties);
					...
					if (!Config.SYSTEM_DOCUMENT_NAME_MISMATCH_CHECK || PathUtils.getName(path).equals(fileName)) {
						...
						if (Config.REPOSITORY_NATIVE) {
							if (!PathUtils.getName(path).equals(fileName)){
								String parentPath = PathUtils.getParent(path);
								String newPath = parentPath + "/" + fileName;
								vtProperties.put("newPath", newPath);
								vtProperties.put("newName", fileName);
							}
							new DbDocumentModule().checkin(null, path, is, size, comment, null, increaseVersion, vtProperties);
				...
				if (action == UIFileUploadConstants.ACTION_DIGITAL_SIGNATURE_INSERT
					...
					switch (action) {
						case UIFileUploadConstants.ACTION_DIGITAL_SIGNATURE_INSERT:
							Document newDoc = new Document(vtProperties);
						...
						case UIFileUploadConstants.ACTION_DIGITAL_SIGNATURE_UPDATE:
							OKMDocument.getInstance().checkout(null, path);
							OKMDocument.getInstance().checkin(null, path, fis, "Signed", new HashMap<>());
	 ...
	 */
	private synchronized String importZip(String path, InputStream is, HashMap<String, Object> vtProperties) throws PathNotFoundException, ItemExistsException,
		...
		try {
			...
			ImpExpStats stats = RepositoryImporter.importDocuments(null, tmpOut, path, false, false, false, out, new TextInfoDecorator(tmpOut), vtProperties);
	 ...
	 */
	private synchronized String importJar(String path, InputStream is, HashMap<String, Object> vtProperties) throws PathNotFoundException, ItemExistsException,
			AccessDeniedException, RepositoryException, IOException, DatabaseException, ExtensionException, AutomationException {

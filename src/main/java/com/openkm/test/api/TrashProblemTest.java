...
public class TrashProblemTest extends TestCase {
	...
	public void testTrashProblem() throws AccessDeniedException, RepositoryException, PathNotFoundException, ItemExistsException,
		...
		okmDocument.checkin(token, BASE + "/alpha/one.txt", new ByteArrayInputStream("new version 1".getBytes()), "version 1", new HashMap<>());
		...
		okmDocument.checkin(token, BASE + "/alpha/two.txt", new ByteArrayInputStream("new version 1".getBytes()), "version 1", new HashMap<>());
		okmDocument.checkout(token, BASE + "/alpha/two.txt");
		okmDocument.checkin(token, BASE + "/alpha/two.txt", new ByteArrayInputStream("new version 2".getBytes()), "version 2", new HashMap<>());

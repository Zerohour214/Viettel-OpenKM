...
public class NodeDocumentDAO {
	...
	public synchronized NodeDocumentVersion create(NodeDocument nDoc, InputStream is, long size) throws PathNotFoundException, AccessDeniedException,
		...
		try {
			...
			newDocVer.setOldName(nDoc.getName());
	...
	public List<NodeDocument> findByParentWithOffset(String parentUuid, int offset, int limit) throws PathNotFoundException, DatabaseException {
		log.debug("findByParentWithOffset({}, {}, {})", parentUuid, offset, limit);
		String qs = "from NodeDocument nd where nd.parent=:parent order by nd.name";
		Session session = null;
		Transaction tx = null;
		...
		try {
			...
			SystemProfiling.log(parentUuid, System.currentTimeMillis() - begin);
			log.trace("findByParentWithOffset.Time: {}", System.currentTimeMillis() - begin);
			log.debug("findByParentWithOffset: {}", ret);
			return ret;
		} catch (PathNotFoundException e) {
			HibernateUtil.rollback(tx);
			throw e;
	...
	public int countDocumentChildTotal(String parentUuid) throws PathNotFoundException, DatabaseException {
		log.debug("countDocumentChildTotal({})", parentUuid);
		String qs = "select count(*) from NodeDocument nd where nd.parent=:parent";
		Session session = null;
		Transaction tx = null;

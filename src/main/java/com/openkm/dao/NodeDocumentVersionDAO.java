...
public class NodeDocumentVersionDAO extends GenericDAO<NodeDocumentVersion, String> {
	 ...
	 */
	public NodeDocumentVersion checkin(String user, String comment, String docUuid, InputStream is, long size, int increment, HashMap<String, Object> vtProperties)
		...
		try {
			...
			if(vtProperties.get("newName") != null && !vtProperties.get("newName").toString().trim().isEmpty()){
				newDocVersion.setOldName(vtProperties.get("newName").toString().trim());
			}
			...
			// Set vt properties
			if (vtProperties.get("docCode") != null && !vtProperties.get("docCode").equals("")){
				nDoc.setDocCode(vtProperties.get("docCode").toString());
			}
			if (vtProperties.get("docName") != null && !vtProperties.get("docName").equals("")){
				nDoc.setDocName(vtProperties.get("docName").toString());
			}
			if (vtProperties.get("docEffectiveDate") != null){
				nDoc.setDocEffectiveDate((Calendar) vtProperties.get("docEffectiveDate"));
			}
			if (vtProperties.get("docOrgId") != null && !vtProperties.get("docOrgId").toString().trim().isEmpty()){
				nDoc.setDocOrgId(Long.parseLong(vtProperties.get("docOrgId").toString()));
			}
			if (vtProperties.get("docConfidentiality") != null && !vtProperties.get("docConfidentiality").toString().trim().isEmpty()){
				nDoc.setDocConfidentiality(Integer.parseInt(vtProperties.get("docConfidentiality").toString()));
			}
			if (vtProperties.get("docExpiredDate") != null){
				nDoc.setDocExpiredDate((Calendar) vtProperties.get("docExpiredDate"));
			}
			if (vtProperties.get("docAuthor") != null && !vtProperties.get("docAuthor").toString().trim().isEmpty()){
				nDoc.setDocAuthor(vtProperties.get("docAuthor").toString());
			}
			if (vtProperties.get("publishCom") != null && !vtProperties.get("publishCom").toString().trim().isEmpty()){
				nDoc.setPublishCom(vtProperties.get("publishCom").toString());
			}
			if (vtProperties.get("publishedYear") != null){
				nDoc.setPublishedYear((Integer) vtProperties.get("publishedYear"));
			}
			if (vtProperties.get("pageNumber") != null){
				nDoc.setPageNumber((Integer) vtProperties.get("pageNumber"));
			}
			if(vtProperties.get("newPath") != null && !vtProperties.get("newPath").toString().trim().isEmpty()){
				nDoc.setPath(vtProperties.get("newPath").toString().trim());
			}
			if(vtProperties.get("newName") != null && !vtProperties.get("newName").toString().trim().isEmpty()){
				nDoc.setName(vtProperties.get("newName").toString().trim());
			}
	...
	public void restoreVersion(String docUuid, String versionId) throws PathNotFoundException, AccessDeniedException, LockException,
		...
		try {
			...
			if(namDocVersion.getOldName() != null && !namDocVersion.getOldName().trim().isEmpty()){
				nDoc.setName(namDocVersion.getOldName().trim());
				String parentPath = PathUtils.getParent(nDoc.getPath());
				nDoc.setPath(parentPath + "/" + namDocVersion.getOldName().trim());
			}
			session.update(nDoc);

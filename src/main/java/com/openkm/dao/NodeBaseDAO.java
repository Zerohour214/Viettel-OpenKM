...
public class NodeBaseDAO {
	...
	public void addKeyword(String uuid, String keyword) throws PathNotFoundException, AccessDeniedException, DatabaseException {
		...
		try {
			...
			SecurityHelper.checkRead(node);
			SecurityHelper.checkWrite(node);
			System.out.println("KEYWORD NODE " + node.getKeywords().toString());
	...
	public void updateKeyword(String uuid) throws PathNotFoundException, AccessDeniedException, DatabaseException {
		log.debug("updateKeyword({})", uuid);
		Session session = null;
		Transaction tx = null;
